(function(jQuery){
	...
	jQuery.fn.jparallax=function(options){
		...
		return this.each(function(){
			...
			console.log(viewport);
			var mouseport=jQuery.extend({},{element:viewport.element},settingsMouseport,{xinside:false,yinside:false,active:false,ontarget:false});
			var layer=[];
			...
			for(var i=0;i<viewport.element.children().length;i++){
				layer[i]=jQuery.extend({},settingsLayer,layersettings[i],{element:viewport.element.children('*:eq('+i+')')});
